...
<!-- #HEADER -->
<header id="header">
    <div id="logo-group" onclick="window.open('${smartSource}/index.html');">
    ...
    <nav>
        ...
        <ul>
            ...
            <c:choose>
                ...
                <c:otherwise>
                    <c:set var="menulList" value="${sys:getUserMenuList()}"/>
                </c:otherwise>
            </c:choose>
            <c:forEach items="${menulList}" var="menu" varStatus="idxStatus">
                <c:if test="${not empty menu and menu.type eq '0'}">
                    <li><c:choose>
                                ...
                                <c:forEach items="${menulList}" var="menu2">
                                    <c:if test="${menu2.type ne '0' and menu2.parent.code eq menu.code}">
                                        <li><c:choose>
                                            ...
                                            <c:otherwise>
                                                <a data-id="${menu2.code}"
                                                   href="${(fn:indexOf(menu2.url, '://') eq -1 ? ctx.concat(menu2.url) : '')}">
                                                    <i class="fa fa-fw ${menu2.icon}"></i> <span>${menu2.name}</span>
                                                </a>
                                            </c:otherwise>
                                        </c:choose></li>
                                    </c:if>
...
<div class="page-footer">
    <div class="row">
        <div class="col-xs-12 col-sm-6">
				<span class="txt-color-white">${fns:getProperty('platform')}
